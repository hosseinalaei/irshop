...
const AddGroup = () => {
  ...
  // useEffect(() => {
  //   if (items2?.length > 0) {
  //     let filteredArray1 = items2.filter(
  //       (obj1) => !items1.some((obj2) => obj2.id === obj1.id)
  //     );

  //     setUpdatedArr(filteredArray1);

  //     // Now filteredArray1 contains objects from array1 that don't have the same 'id' in array2
  //     console.log("filteredArray1filteredArray1filteredArray1", filteredArray1);
  //   }
  // }, [items1]);
  ...
  const addGroupSpec = () => {
    const body = {
      id: uuidv4(),
      isDelete: false,
      attributesId: items1?.map((value) => value?.id),
      name: groupName,
    };
    // axiosService .post("/Specification/registerSpecification", body)
    httpRequest({
      url: "/Specification/registerAttributeGroup",
      method: "POST",
      data: body,
    }).then((res) => {
  ...
  const updateGroup = () => {
    ...
    const requestBody = {
      ...
      attributesId: items1.map((item) => item?.id),
      name: groupName,
    };
